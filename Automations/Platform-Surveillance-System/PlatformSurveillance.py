...
#                 including PID, name, username, status, CPU usage and RAM usage.
#
#-----------------------------------------------------------------------------------------------------------
...
# Description   : Collects CPU, RAM, network and running process information
...
def PlatFormSurveillance(FolderName):
    ...
    for info in Data:
        ...
        fd.write(f"PID      : {info.get('pid')}\n")
        fd.write(f"Name     : {info.get('name')}\n")
        fd.write(f"Username : {info.get('username')}\n")
        fd.write(f"Status   : {info.get('status')}\n")
        fd.write(f"CPU      : {info.get('cpu_percent')}%\n")
        fd.write(f"RAM      : {info.get('ram_percent'):.2f}%\n")
...
# Output        : Displays information or starts the scheduler
