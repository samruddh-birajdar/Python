...
# Description  : Monitors system information such as CPU, RAM, network usage
#                and running processes, and maintains the information in log files.
...
# Output        : Running process information into log file
...
def ProcessScan(fd):
    ...
    for proc in psutil.process_iter():
        try:
            ...
            fd.write(f"PID: {info['pid']} | Name: {info['name']} | "
                     f"Status: {info['status']} | CPU: {info['cpu_percent']}% | "
                     f"RAM: {info['ram_percent']:.2f}%\n")
...
def PlatFormSurveillance(FolderName):
    ...
    FileName = os.path.join(FolderName, f"_{timestamp}.log")
    ...
    fd.write(Border + "\n")
    fd.write("----------  Platform Survillience System ---------\n")
...
def main():
    ...
    print(Border)
    print("----------  Platform Survillience System -----------")
    print(Border)
